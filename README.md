...
Two more cases route to review rather than resubmit. A typed Form B whose answers are shifted half a row puts two answers inside one field's region and none in the next, so a blank field directly under a field holding two rows of answers is flagged for a look. And on the medical lines, where a dash or a tick is a real answer, any vector mark at all is enough to hold the field for review instead of calling it blank.
