...
A signature placed well below its label, as happens when a student drops a drawing into a Google Doc under the line, is not found. Two submissions did this. They go to `incorrect` rather than `not_sure`, which is the wrong bin, and a region below the label was tried and rejected because it caught the next field's content on every other form.

Photos taken at an angle, or with a hand or shadow across the page, sometimes defeat the label matcher. Those go to `not_sure`, which is where they belong. A name written in grey so light that it falls under the ink threshold reads as blank, and a screenshot of a Docs page with very wide line spacing can put a typed name out of reach. One of each in the first batch.
