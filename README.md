...
Pixel darkness, last. Ink that survives the erasing of rules and printed text is measured as a fraction of the field. It never passes a field on its own. It only decides whether an unreadable field goes to `incorrect` or to `not_sure` with a note to look by eye. Below the floor there is nothing on the line. Above it, something is there that no reader could turn into text, and a person should look.
...
Photos taken at an angle, or with a hand or shadow across the page, sometimes defeat the label matcher. Those go to `not_sure`, which is where they belong. A very small or very faint signature on a scan can read as blank when the crop OCR finds no token and the ink fraction stays under the floor.
...
Tested against thirteen labelled samples covering blank templates, stylus signatures, typed fields, scans, a phone photo, a Google Docs screenshot, and signature pages submitted on their own, plus corrupt, empty, and unrelated files. Every one lands in the right folder, and that set is rerun after every change. The first real batch of 615 forms was audited by eye afterwards, which is where most of the rules above came from. Of the 219 the first version called incorrect, 76 were fine and one form it passed had a blank name and date. Both kinds of mistake are fixed in the current version, and a sample of each folder is still worth a look after any run.
...
`probe` looks inside those regions and measures four things. `ink_in` counts vector path points that land in the region, after `paintable` has thrown out anything that would not show on paper. `overlays_in` counts annotations, filled widgets, and small pasted images that put ink inside the region, using the per annotation pixel masks that `page_overlays` built once for the page. `text_in` collects OCR or native words in the region above a confidence floor of 60, drops boilerplate words like "signature" and "parent", and joins what is left. `dark_frac` crops the region from the rendered page, erases long thin horizontal runs (the printed rule) and any high confidence printed words, and reports what share of the remaining pixels is dark. When a scan gives no token at any confidence and the crop is not empty, `crop_ocr` runs Tesseract again on that crop alone, upscaled and in single line mode, and accepts the result only if a token has four letters, two digits, or is a variant of N/A. Signatures are allowed to use the region beside and above their line, and the best one wins on the tuple (ink, overlays, text length, darkness). Text fields only get their preferred region, because the text above a field is the previous line of the form, not an answer. For the fallback region of a signature, text is discarded for the same reason and only ink counts. Text fields and dates ask for the majority of an annotation's ink to fall inside the region, so a signature stamp riding high does not fill the name line above it.
...
`analyse` runs the above for one file and returns a dictionary. For each field in the spec it tries each page in order until a label locates, then probes. A signature entry records `signed` (five or more ink points, any OCR token at all, or an overlay with ink in the region), `ink_unverified` (a scan with an ink fraction over the signature floor but nothing any reader could turn into text), and `maybe_ink` (one to four stray path points in a vector PDF). It then looks for a "Date" label within about four label heights of the signature and probes that too, one text row higher than usual, and only beside the label on forms where answers go to the right. A text entry records `filled`, which needs twenty five ink points rather than five, and its own `ink_unverified` at a lower floor. The result also lists which pages carried fields, which content pages carried none, and any pages skipped as separators. With `--debug` every probe prints its numbers, which is how you find out why a particular form was misread.
...
Tuning constants sit near the top of the file. `INK_MIN` is five path points for a signature and `TEXT_INK_MIN` twenty five for anything else. `DARK_UNVERIFIED` and `TEXT_DARK_UNVERIFIED` are the ink fractions above which an unreadable field goes to review instead of to resubmit, and `CROP_OCR_MIN_DARK` is the fraction below which the crop is not worth sending to Tesseract. `OVERLAY_MIN_PIXELS` is how much annotation ink has to land in a region, and `OVERLAY_INSIDE` how much of it, for text fields. `OCR_DPI` is 150. `OCR_IF_TEXT_UNDER` and `SCAN_IMAGE_COVER` decide when a page gets OCR. `DATE_EXTRA_UP` is the extra reach for dates. Change these with the `regress/` set in front of you, because each was arrived at by watching a specific sample go wrong, and most of them moved during the first audit.

Known gaps. A signature dropped well below its label is not found, and a very faint or tiny signature on a scan can still read as blank. Both are listed under "Known limits" above.
